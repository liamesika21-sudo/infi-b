...
/* Shared rendering for "Max Mahlin" instructor notes — used both by the
   /instructor-notes page and embedded per-week on /weeks/[n]. */

type WeeklyInsight = MaxInsightsData["weeklyInsights"][number];
type CommonMistake = MaxInsightsData["commonMistakesFromMax"][number];

export interface MaxWeekGroup {
  week: number;
  topic?: string;
  recitations: number[];
...
export function buildWeekGroups(insights: MaxInsightsData): MaxWeekGroup[] {
...
export function WeekSidebar({ groups }: { groups: MaxWeekGroup[] }) {
  return (
...
export function WeekSection({ group }: { group: MaxWeekGroup }) {
  return (
